...
public class DiffAnalyzer {
    public static void main(String[] args) {
        ...
        Driver driver = GraphDatabase.driver("bolt://localhost:7687", AuthTokens.basic("neo4j", "smart"));
        Session session = driver.session();

//        List<ActionCluster> actionClusters = RepoAnalyzer.generateActionClusters(filePair.getOldContent(), filePair.getNewContent());
...
//        params.put("endLine", rootAction.getNode().getEndLine());
        session.run("CREATE (a:Action {operation: $operation, nodeType: $nodeType, label:$label," +
                        " startLine:$startLine, endLine: $endLine})",
                params);

        session.run("CREATE (a:Person {name: {name}, title: {title}})",
                parameters("name", "Arthur001", "title", "King001"));
        StatementResult result = session.run("CREATE (baeldung:Company {name:$name}) " +
